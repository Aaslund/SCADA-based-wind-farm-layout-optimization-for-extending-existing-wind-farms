...
figure_folder = f'D:/Thesis/Figures/Penmanshiel/Weibull/{date_folder}/{n}_winddirections/'
if season_bool:
...
for year in year_list:
    figure_folder_year =f"{figure_folder}/{year}/ref/"
    os.makedirs(figure_folder_year, exist_ok=True)
    df_hourly = pd.read_csv(f'{load_folder}df_windspeed_all_{year}.csv')
    ...
    df_hourly['month'] = df_hourly['date_time'].dt.month  # Extracts month (1-12)
    ...
    for season in season_list:
        ...
        for wind_dir_i in bin_labels:
            ...
            df_filtered_1stamp = df_filtered.groupby('date_time')[['wind_speed', 'TI']].median()
            wind_dir_probability = df_filtered['date_time'].nunique()/total_points_season
            ...
            if not df_filtered.empty:
                ...
                plt.figure()
                print(f'{season}')
                ks,p_value, rmse, tail_error = evaluate_weibull_fit2(df_filtered_1stamp['wind_speed'], k_mle, loc_mle, A_mle, season, wind_dir_i)
                # Store results
                ...
                figsize=(8,6)
                filename=f"{year}_wd{int(wind_dir_i)}.png"
                save_path = os.path.join(figure_folder15, filename)
                plot_weibull_fit(df_filtered, k_mle, A_mle, season, wind_dir_i,
    ...
    df_total_results.to_csv(f'{save_folder}/weibull_results_{year}.csv')