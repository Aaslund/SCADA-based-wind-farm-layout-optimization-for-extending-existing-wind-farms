# -*- coding: utf-8 -*-
"""
Created on Mon Jun  2 03:16:41 2025
...
figure_folder = f'D:/Thesis/Figures/Penmanshiel/Weibull/{date_folder}/consensus/{n}_winddirections/'
if season_bool:
...
for season in season_list:
    ...
    for wind_dir_i in bin_labels:
        ...
        count_per_timestamp = df_bin.groupby('date_time')['turbine_id'].nunique()
        #print(f'count_per_timestamp:{len(count_per_timestamp)}')
        valid_timestamps = count_per_timestamp[count_per_timestamp >= 0.6*sum(mask)].index
        #print(f'valid_timestamps:{len(valid_timestamps)}')
        df_valid = df_bin[df_bin['date_time'].isin(valid_timestamps)]
        total_points_season += df_valid['date_time'].nunique()      
    for wind_dir_i in bin_labels:
        all_wind_speeds = []
        all_weights = []
        # Compute weights for the current wind direction bin center
...
save_folder = load_folder+f'/Weibull/{n}_winddirections/consensus/'
