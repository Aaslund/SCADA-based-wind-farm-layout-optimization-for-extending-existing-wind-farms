...
# AEP validation 3
    ...
    for i in range(1,n_wt+1):
        ...
        mask2=metadata[metadata['turbine_id']==i]
        output_frac[i-1]=float(mask2['active_len'].sum())/float(mask2['initial_len'].sum())
        aep_pywake_adj=aep_pywake_i*output_frac[i-1]
    ...
    output_frac[n_wt] = metadata['active_len'].sum()/metadata['initial_len'].sum()
    aep_pywake_adj = aep_pywake*output_frac[n_wt]
    ...
    aep_tot_results_df.to_csv(f'{save_folder}/aep_results_{n}winddirections_{season_txt}_frac.csv')