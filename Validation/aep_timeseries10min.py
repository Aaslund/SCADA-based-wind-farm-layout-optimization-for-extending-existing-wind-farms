# -*- coding: utf-8 -*-
"""
Created on Thu May 29 15:32:09 2025
...
start_date = pd.to_datetime('2017-09-03 00:00:00')
end_date = pd.to_datetime('2017-09-08 23:50:00')
nmeas = int((end_date - start_date) / pd.Timedelta(minutes=10))+1
...
ax.plot(df_turb['date_time'],df_turb['power'] , '-', label='Measured Power')

# Plot simulated power with aligned time
ax.plot(time_stamp, sim_res.Power.sel(wt=nturbine_i-1), '-',label='Simulated Power')

#ax.set_xlim(pd.Timestamp("2017-09-03"), pd.Timestamp("2017-09-08"))

rec_sum = df_turb['power'].sum()/6* 1e-3  # MWh
sim_sum = float(sim_res.Power.sel(wt=nturbine_i-1).sum())/6 * 1e-3  # MWh
...
ax2.plot(df_turb['date_time'], df_turb['wind_speed'], label='Wind Speed (m/s)', color='magenta')
ax2.set_ylabel("Wind Speed (m/s)", color='magenta')
ax2.tick_params(axis='y', labelcolor='magenta')

# Right y-axis: Wind direction
ax3 = ax2.twinx()
ax3.plot(df_turb['date_time'], df_turb['wind_dir'], label='Wind Direction (°)', color='tab:green')
...
# Group by time and sum power from all turbines
df_total = df_all.groupby('date_time', as_index=False)['power'].sum()

# Create plot
fig, ax = plt.subplots(figsize=(16, 4))

# Plot actual total power
ax.plot(df_total['date_time'], df_total['power']*1e-3, '-', label='Measured Power')

# Sum simulated power across turbines (dimension 'wt') for each time step
sim_total_power = sim_res.Power.sum(dim='wt')

# Plot simulated total power
ax.plot(time_stamp, sim_total_power*1e-3, '-', label='Simulated Power')

# Compute energy totals
rec_sum = df_total['power'].sum() / 6 * 1e-3  # kWh (assuming 10 min intervals: 6 per hour)
sim_sum = float(sim_total_power.sum()) / 6 * 1e-3  # kWh
