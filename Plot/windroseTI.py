# -*- coding: utf-8 -*-
"""
Created on Thu May 29 20:22:50 2025
...
def plot_ti_rose(df, ti_bins, ti_labels, save_path, nplot, rmax):
    # Ensure wind direction is within [0, 360)
    df['wind_dir'] = df['wind_dir'] % 360

    # Create figure and WindroseAxes
    fig = plt.figure(figsize=(9, 9))
    ax = WindroseAxes.from_ax(fig=fig)
    ax.bar(df['wind_dir'], df['TI'], bins=ti_bins, normed=True,
           opening=0.8, edgecolor='white', cmap=cm.plasma, nsector=nplot)

    # Configure legend
    legend = ax.set_legend(title="Turbulence Intensity", bbox_to_anchor=(1.25, 0), loc='lower right', prop={'size': 15})
    for t, label in zip(legend.get_texts(), ti_labels):
        t.set_text(label)
        t.set_fontsize(15)

    # Radial axis limits and labels
    ax.set_rlim((0, rmax))
    ax.set_rgrids([rmax / 5 * i for i in range(1, 6)],
                  [str(round(rmax / 5 * i, 1)) for i in range(1, 6)])

    # Export binned frequency table to CSV
    #frequencies = ax._info['table']
    #bin_centers = np.arange(0, 360, 360 // nplot)
    #wind_dir_bins = [f"{d}°" for d in bin_centers]
    #df_out = pd.DataFrame(frequencies, index=ti_labels, columns=wind_dir_bins)
    #df_out.loc['Total'] = df_out.sum(axis=0)

    #os.makedirs(os.path.dirname(save_path), exist_ok=True)
    #df_out.to_csv(save_path.replace('.png', '.csv'))
...
TI_bins = [0, 0.05, 0.1, 0.15, 0.2]
TI_labels = ['0.00-0.05', '0.05-0.10', '0.10-0.15', '0.15-0.20', '>0.20']
...
figure_folder = f'D:/Thesis/Figures/Penmanshiel/TI/{date_folder}/'
os.makedirs(figure_folder, exist_ok=True)

#%% === PLOT before WEIGHTS ===
plot_ti_rose(df_hourly, TI_bins, TI_labels, os.path.join(figure_folder, f'TIrose_before_weights{n}_winddirections_{nplot}nplot.png'),nplot,rmax)
...
plot_ti_rose(df_weighted, TI_bins, TI_labels, os.path.join(figure_folder, f'windrose_after_weights_{n}_winddirections_{nplot}nplot.png'),nplot,rmax)