...
if __name__ == '__main__':
    ...
    figure_folder = f"D:/Thesis/Figures/Penmanshiel/Optimization/Whole_farm/{datetime.now().strftime('%Y-%m-%d')}/"
    os.makedirs(figure_folder, exist_ok=True)
    save_folder = load_folder+'Optimization/Whole_farm/'
    ...
    y = np.array([32.04702276, -245.27312429,  414.66876535,  120.11340467,
                  ...
                  315.37169586,   27.71042063])-32.0470227
    ...
    weibull_results = pd.read_csv(
        f'{load_folder}/Weibull/{n}_winddirections/{season_txt}/weibull_results_tot.csv')
    ...
    wfm = PropagateUpDownIterative(site, my_wt, wdm,
                                   blockage_deficitModel=Rathmann(
    ...
    aep_comp = CostModelComponent(input_keys=[
                                  'x', 'y'], n_wt=n_wt, cost_function=aep_func, objective=True, maximize=True)
    ...
    driver = EasyScipyOptimizeDriver(optimizer='COBYLA', maxiter=500, tol=1e-4)
    # %%
    ...
    results_df.to_csv(f'{save_folder}/optimization_{n}winddirections_{season_txt}_iter{recorder.num_cases}.csv', index=False)
    ...
    for i in range(num_iterations):
        for t in range(num_turbines):
            data.append({
                ...
                'x': rec_dict['x'][i][t],
                'y': rec_dict['y'][i][t],
                'AEP': -rec_dict['c'][i]*1e3  # Negate if your cost is -AEP
            })

    # Convert to DataFrame
    turbine_positions_df = pd.DataFrame(data)
    turbine_positions_df.to_csv(f'{save_folder}/turbine_positions_{n}winddirections_{season_txt}_iter{recorder.num_cases}.csv', index=False)